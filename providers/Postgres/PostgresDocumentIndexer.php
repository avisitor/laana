...
class PostgresDocumentIndexer
{
    ...
    public function run(): array
    {
        ...
        while (true) {
            ...
            if (empty($batch)) {
                // Premature exit: no further candidates from iterator
            ...
            // Write immediate JSON snapshot of intended IDs (prefer full intent list if known and capped)
            if ($this->outJson) {
                $ids = !empty($this->intentIds)
                    ? $this->intentIds
                    : array_map(function($r){ return (int)$r['sourceid']; }, $batch);
            ...
            // Overwrite JSON snapshot with current progress after batch
            if ($this->outJson) {
                $dir = dirname($this->outJson);
                if (!is_dir($dir)) { @mkdir($dir, 0775, true); }
                $snapshot = [
